...
import { desactivateUser, getAllUsers } from "@/helpers/admin.helper"
...
const GetAllUsers = () => {
  ...
  const handleDeactivateUser = async (userEmail: string) => {
    try {
      ...
      await desactivateUser(userEmail, token)
      setUsers((prevUsers) => prevUsers.map((user) => (user.email === userEmail ? { ...user, isActive: false } : user)))
    } catch (err: any) {
      ...
      <div className="flex-1 md:ml-64">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-4 md:py-6">
          <div className="bg-white rounded-lg shadow-sm p-4 md:p-6">
            <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-4 md:mb-6">Users</h2>
            {loading ? (
                  ...
                  <table className="w-full border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {currentUsers.length ? (
                          ...
                          <tr key={user.email} className="hover:bg-gray-50">
                            ...
                            <td className="px-2 md:px-4 py-2 md:py-3">
                              <button
                                ...
                                className={`${
                                  !user.isActive ? "bg-gray-400 cursor-not-allowed" : "bg-red-500 hover:bg-red-600"
                        ...
                        <tr>
                          <td colSpan={5} className="px-4 py-8 text-center text-gray-500 text-sm md:text-base">
                            No users available.
                          </td>
                        </tr>
                      )}
