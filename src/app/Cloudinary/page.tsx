...
interface FileUploadProps {
  userprops?: UserProps; // Hacemos opcional userprops para evitar errores si no se pasa
}
...
const FileUploadComponent: React.FC<FileUploadProps> = ({ userprops }) => {
  ...
  const [file, setFile] = useState<File | null>(null);
  const [uploading, setUploading] = useState(false);
  const [fileUrl, setFileUrl] = useState(userprops?.image_url || defaultImage);
  const [showModal, setShowModal] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);
  ...
  const handleFileUpload = async () => {
    ...
    if (!userprops?.email) {
      alert("No se puede subir la imagen sin un correo electrónico válido.");
      return;
    }
