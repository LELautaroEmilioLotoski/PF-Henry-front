...
import { useUserContext } from "@/context/UserContext";  // Asegúrate de tener el contexto de usuario

const EmployeeReservations = () => {
  const { token, setToken } = useUserContext();  // Accede al token desde el contexto
  const [reservations, setReservations] = useState<IReservations[]>([]);
  const [email, setEmail] = useState("");

  useEffect(() => {
    // Verificar si el token está disponible en el contexto
    console.log("Token desde el contexto:", token);

    // Si el token no está disponible en el contexto, intenta cargarlo desde localStorage
    if (!token) {
